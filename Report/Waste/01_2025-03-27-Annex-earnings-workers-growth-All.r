# Multi-Country Labor Market Analysis (2016-2024) - Version 2
...
output_dir <- "C:/Users/wb593225/OneDrive - WBG/Desktop/Luis - Private/Lablac/Semiannual report/Excel/Lablac output/01_New outputs/General Lablac Checks"
...
process_dataset <- function(file, period_label, sector_var = "sector") {
  ...
  # Apply base filters: edad > 14 and cohi == 1
  dat <- dat %>% filter(edad > 14, cohi == 1)
  ...
  # Get sector from the appropriate variable
  workers$sector_value <- workers[[sector_var]]
  
  # Get sector labels if available
  ...
  total_skill <- data.frame(
    skill_level = "Total",
    total_workers = sum(skill_results$total_workers, na.rm = TRUE),
    mean_ila = calculate_weighted_mean(workers$ila_ppp17[!is.na(workers$skill_level)], 
                                     workers$pondera[!is.na(workers$skill_level)]),
    mean_wage = calculate_weighted_mean(workers$wage_ppp17[!is.na(workers$skill_level)], 
                                      workers$pondera[!is.na(workers$skill_level)])
  )
  ...
  total_gender <- data.frame(
    gender = "Total",
    total_workers = sum(gender_results$total_workers, na.rm = TRUE),
    mean_ila = calculate_weighted_mean(workers$ila_ppp17[!is.na(workers$gender)], 
                                     workers$pondera[!is.na(workers$gender)]),
    mean_wage = calculate_weighted_mean(workers$wage_ppp17[!is.na(workers$gender)], 
                                      workers$pondera[!is.na(workers$gender)])
  )
  gender_results <- bind_rows(gender_results, total_gender)
  
  # 3. Calculate total weighted workers by sector
  ...
  # 3a. Add sector labels
  sector_results$sector_label <- names(sector_labels)[match(sector_results$sector_value, sector_labels)]
  
  # 3b. Add total row for sector
  total_sector <- data.frame(
    sector_value = NA,  # No numeric sector for total
    total_workers = sum(sector_results$total_workers, na.rm = TRUE),
    mean_ila = calculate_weighted_mean(workers$ila_ppp17[!is.na(workers$sector_value)], 
                                     workers$pondera[!is.na(workers$sector_value)]),
    mean_wage = calculate_weighted_mean(workers$wage_ppp17[!is.na(workers$sector_value)], 
                                      workers$pondera[!is.na(workers$sector_value)]),
    sector_label = "Total"
  )
  ...
  gender_sector_results <- workers %>%
    filter(!is.na(gender), !is.na(sector_value)) %>%
    group_by(gender, sector_value) %>%
    summarize(
      total_workers = sum(pondera, na.rm = TRUE),
      mean_ila = calculate_weighted_mean(ila_ppp17, pondera),
      mean_wage = calculate_weighted_mean(wage_ppp17, pondera),
  ...
  # 4a. Add sector labels
  gender_sector_results$sector_label <- names(sector_labels)[match(gender_sector_results$sector_value, sector_labels)]
...
process_country <- function(country_code, config) {
  ...
  results_start <- process_dataset(start_files[1], start_label, config$sector_var)
  results_end <- process_dataset(end_files[1], end_label, config$sector_var)
  ...
  workers_by_sector <- results_start$sector %>%
    select(sector_value, sector_label, total_workers_start = total_workers) %>%
    left_join(
      results_end$sector %>% select(sector_value, total_workers_end = total_workers),
      by = "sector_value"
    ) %>%
    mutate(
      annualized_change = mapply(
  ...
  ila_by_sector <- results_start$sector %>%
    select(sector_value, sector_label, ila_start = mean_ila) %>%
    left_join(
      results_end$sector %>% select(sector_value, ila_end = mean_ila),
      by = "sector_value"
    ) %>%
    mutate(
      annualized_change = mapply(
  ...
  wage_by_sector <- results_start$sector %>%
    select(sector_value, sector_label, wage_start = mean_wage) %>%
    left_join(
      results_end$sector %>% select(sector_value, wage_end = mean_wage),
      by = "sector_value"
    ) %>%
    mutate(
      annualized_change = mapply(
  ...
  male_workers_by_sector <- results_start$gender_sector %>%
    ...
    left_join(
      results_end$gender_sector %>% 
        filter(gender == "Male") %>%
        select(sector_value, total_workers_end = total_workers),
      by = "sector_value"
    ) %>%
    mutate(
      annualized_change = mapply(
  ...
  male_ila_by_sector <- results_start$gender_sector %>%
    ...
    left_join(
      results_end$gender_sector %>% 
        filter(gender == "Male") %>%
        select(sector_value, ila_end = mean_ila),
      by = "sector_value"
    ) %>%
    mutate(
      annualized_change = mapply(
  ...
  male_wage_by_sector <- results_start$gender_sector %>%
    ...
    left_join(
      results_end$gender_sector %>% 
        filter(gender == "Male") %>%
        select(sector_value, wage_end = mean_wage),
      by = "sector_value"
    ) %>%
    mutate(
      annualized_change = mapply(
...
# Create the output Excel file with V2 added to the name
today_date <- format(Sys.Date(), "%Y-%m-%d")
output_file <- file.path(output_dir, paste0(today_date, "-labor-market-analysis-V2.xlsx"))
