...
process_dataset <- function(file, period_label, sector_var = "sector") {
  ...
  sector_results <- sector_results %>%
    left_join(sector_income_results, by = "sector_value")
  ...
  total_sector <- data.frame(
    ...
    mean_ila = calculate_weighted_mean(income_workers$ila_ppp17[!is.na(income_workers$sector_value)], 
                                       income_workers$pondera[!is.na(income_workers$sector_value)]),
    mean_wage = calculate_weighted_mean(income_workers$wage_ppp17[!is.na(income_workers$sector_value)], 
                                        income_workers$pondera[!is.na(income_workers$sector_value)]),
    sector_label = "Total"
  )
  ...
  gender_sector_results <- gender_sector_results %>%
    left_join(gender_sector_income_results, by = c("gender", "sector_value"))
