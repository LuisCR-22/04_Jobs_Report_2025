# Informality Series Analysis for Peru and Paraguay (Urban Areas Only)
# This script processes LABLAC datasets to create a time series of informality measures
# Modified to focus only on Peru and Paraguay, and filter for urban areas only
...
output_file <- file.path(output_dir, paste0("03_", current_date, "-pry-per-urban-informality-series.xlsx"))
...
list_files_to_process <- function() {
  ...
  # Filter only for Peru and Paraguay files between 2016 Q1 and 2024 Q4
  files_to_process <- grep("LABLAC_(per|pry)_(20(1[6-9]|2[0-4]))_q\\d{2}", basename(all_files), value = TRUE)
...
process_file <- function(file_path) {
  ...
  required_vars <- c("ocupado", "pais_ocaux", "pondera", "edad", "hombre", "asal", "urbano")
  
  # For legal informality in Peru, we need dsegsale; for Paraguay, we need djubila
  if(country_code == "per") {
    legal_var <- "dsegsale"  # Peru uses health insurance
  } else {
    legal_var <- "djubila"   # Paraguay uses pension
  }
  ...
  # Filter for employed adults (15+) in URBAN areas only
  employed_adults <- dat %>% filter(ocupado == 1, edad > 14, urbano == 1)
...
# -----------------------------------------------------------------------------
# Process files (Peru and Paraguay only, urban areas only)
...
if(!is.null(results) && nrow(results) > 0) {
  ...
  # Display a sample of results for each country
  for(country in unique(results$Country)) {
    ...
    for(i in 1:nrow(country_results)) {
      cat(sprintf("%s, %s: %.2f%%\n", 
                  country_results$Indicator[i], 
                  country_results$Period[i], 
                  country_results$Value[i] * 100))
    }
  }
